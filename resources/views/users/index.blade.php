...
@section('content')
    <div class="col-span-12">
        <div class="flex flex-col gap-y-3 md:h-10 md:flex-row md:items-center">
            <div class="text-base font-medium group-[.mode--light]:text-white"> Data Pengguna </div>
...
@push('scripts')
    ...
    <script>
        ...
        function renderUsers(users) {
            ...
            users.forEach(user => {
                ...
                const statusClass = user.status == 'active' ? 'text-success' : 'text-slate-400'

                const categories = user.ai_keywords?.map(cat => `
        ...
        document.getElementById('filter-status')?.addEventListener('change', () => {
            loadUsers(currentQuery, 1)
        })

        searchInput?.addEventListener('input', e => {
            clearTimeout(debounceTimer)
            debounceTimer = setTimeout(() => loadUsers(e.target.value), 500)
        })
        ...
        function renderPagination(paginator) {
            ...
            const li = (content, page = null, active = false) => `
                <li class="flex-1 sm:flex-initial">
                    <a
                        ${page ? `onclick="loadUsers(currentQuery, ${page})"` : ''}
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            ...
            axios.delete(`/users/${deleteId}/delete`, {
                headers: {
                ...
                if (res.data.status) {
                    ...
                    showToast(
                        'success',
                        'Berhasil',
                        'Pengguna berhasil dihapus. load data terbaru...'
                    )
                    document
