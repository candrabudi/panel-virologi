@extends('layouts.app')
@section('title', 'Ubah Layanan Keamanan Siber')
@section('content')
    ...
    <div class="col-span-12">
        <div class="flex flex-col gap-1 mb-10">
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 font-medium">Sesuaikan detail penawaran keamanan Anda</p>
                </div>
            </div>
        </div>
        ...
        <form id="service-form" class="grid grid-cols-12 gap-8">
                        ...
                        <div class="grid grid-cols-12 gap-6">
                            ...
                            <div class="col-span-12">
                                ...
                                <select name="category"
                                    ...
                                    @foreach (['soc', 'pentest', 'audit', 'incident_response', 'cloud_security', 'governance', 'training', 'consulting'] as $c)
                                        <option value="{{ $c }}"
                                            {{ $cyberSecurityService->category == $c ? 'selected' : '' }}>
                            ...
                            <div class="col-span-12">
                                ...
                                <textarea name="summary" rows="3" required
                                    ...
                                    placeholder="Tinjauan singkat yang menjelaskan nilai yang diberikan layanan ini...">{{ $cyberSecurityService->summary }}</textarea>
                            ...
                            <div class="col-span-12">
                                ...
                                <div class="rounded-xl overflow-hidden border border-slate-200">
                                    <textarea id="content-editor" name="description" rows="15" class="form-control w-full border-none shadow-sm">{{ $cyberSecurityService->description }}</textarea>
                                </div>
                            ...
                            @foreach ($cyberSecurityService->service_scope ?? [] as $item)
                                <div class="flex gap-2 repeater-item items-center mb-3">
                                    <input type="text" value="{{ $item }}"
                                        class="form-control flex-1 rounded-xl border-slate-200 text-sm shadow-sm py-3 px-4 scope-input">
                                    <button type="button"
                                        class="p-3 text-slate-400 hover:bg-danger/10 hover:text-danger rounded-xl remove-item transition-all">
                    ...
                    <div class="box box--stacked p-10 flex flex-col mt-5 hover:border-indigo/30 transition-colors group">
                        ...
                        <div id="deliverables-container" class="space-y-4 flex-1">
                            @foreach ($cyberSecurityService->deliverables ?? [] as $item)
                                <div class="flex gap-2 repeater-item items-center mb-3">
                                    <input type="text" value="{{ $item }}"
                                        class="form-control flex-1 rounded-xl border-slate-200 text-sm shadow-sm py-3 px-4 deliverable-input">
                                    <button type="button"
                                        class="p-3 text-slate-400 hover:bg-danger/10 hover:text-danger rounded-xl remove-item transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                    ...
                    <div class="box box--stacked p-10 flex flex-col mt-5 hover:border-emerald/30 transition-colors group">
                        ...
                        <div id="target-audience-container" class="space-y-4 flex-1">
                            @foreach ($cyberSecurityService->target_audience ?? [] as $item)
                                <div class="flex gap-2 repeater-item items-center mb-3">
                                    <input type="text" value="{{ $item }}"
                                        class="form-control flex-1 rounded-xl border-slate-200 text-sm shadow-sm py-3 px-4 audience-input">
                                    <button type="button"
                                        class="p-3 text-slate-400 hover:bg-danger/10 hover:text-danger rounded-xl remove-item transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                    ...
                    <div class="box box--stacked p-10 flex flex-col mt-5 hover:border-amber/30 transition-colors group">
                        ...
                        <div id="ai-keywords-container" class="space-y-4 flex-1">
                            @foreach ($cyberSecurityService->ai_keywords ?? [] as $item)
                                <div class="flex gap-2 repeater-item items-center mb-3">
                                    <input type="text" value="{{ $item }}"
                                        class="form-control flex-1 rounded-xl border-slate-200 text-sm shadow-sm py-3 px-4 keyword-input">
                                    <button type="button"
                                        class="p-3 text-slate-400 hover:bg-danger/10 hover:text-danger rounded-xl remove-item transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
            ...
            <div class="col-span-12 lg:col-span-4 space-y-8">
                ...
                <div
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-5">
                            ...
                            <h3 class="text-lg font-bold">Panduan Cepat</h3>
                        </div>
                        <ul class="space-y-4 text-sm font-medium">
                            <li class="flex gap-3" style="color: white;">
                                <span
                                    class="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-[10px] font-bold">1</span>
                                <span>Gunakan <b>Nama Lengkap</b> yang mengandung kata kunci keamanan siber.</span>
                            </li>
                            <li class="flex gap-3" style="color: white;">
                                <span
                                    class="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-[10px] font-bold">2</span>
                                <span><b>Ringkasan Eksekutif</b> adalah teks yang pertama kali dibaca user, buatlah
                                    semenarik mungkin.</span>
                            </li>
                            <li class="flex gap-3" style="color: white;">
                                <span
                                    class="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-[10px] font-bold">3</span>
                                <span><b>Field Repeater</b> di sebelah kiri akan tampil sebagai ikon daftar di halaman
                                    produk.</span>
                            </li>
                            <li class="flex gap-3" style="color: white;">
                                <span
                                    class="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-[10px] font-bold">4</span>
                        ...
                        <div
                            ...
                            <div class="form-check form-switch p-0">
                                <input name="is_active" class="form-check-input scale-125 transition-all shadow-sm"
                                    ...
                                    {{ $cyberSecurityService->is_active ? 'checked' : '' }}>
                            </div>
                        </div>
                        <div
                            ...
                            <div class="form-check form-switch p-0">
                                <input name="is_ai_visible" class="form-check-input scale-125 transition-all shadow-sm"
                                    type="checkbox" value="1"
                                    {{ $cyberSecurityService->is_ai_visible ? 'checked' : '' }}>
                            </div>
                        </div>
                        <div>
                    ...
                    <div class="space-y-4">
                        <div
                            ...
                            <div id="thumbnail-preview"
                                class="absolute inset-0 {{ $cyberSecurityService->thumbnail ? '' : 'hidden' }}">
                                <img src="{{ $cyberSecurityService->thumbnail }}" class="w-full h-full object-cover">
                            </div>
                ...
                {{-- AI DOMAIN --}}
                <div class="box box--stacked p-10 flex flex-col mt-5">
                    <h3 class="text-lg font-bold text-slate-700 mb-6">Klasifikasi AI</h3>
                    <div>
                        <label class="block mb-2 text-sm font-bold text-slate-700">Domain AI</label>
                        <input type="text" name="ai_domain" value="{{ $cyberSecurityService->ai_domain }}"
                            class="form-control w-full rounded-xl border-slate-200 py-3 shadow-sm"
                            placeholder="contoh: soc">
                    </div>
                </div>

                {{-- CTA CARD --}}
                <div class="box box--stacked p-10 flex flex-col mt-5">
                    <h3 class="text-lg font-bold text-slate-700 mb-6">Panggilan Aksi (CTA)</h3>
                    <div class="space-y-4">
                        <div>
                            <label class="block mb-2 text-sm font-bold text-slate-700 text-xs text-slate-700">Label
                                Tombol</label>
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-bold text-slate-700 text-xs text-slate-700">URL
                                Tujuan</label>
                ...
                {{-- SEO --}}
                <div class="box box--stacked p-10 flex flex-col mt-5">
                    <h3 class="text-lg font-bold text-slate-700 mb-6">Optimasi SEO</h3>
                    <div class="space-y-4">
                        <div>
                            <label class="block mb-2 text-sm font-bold text-slate-700 text-xs text-slate-700">Judul
                                Meta</label>
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-bold text-slate-700 text-xs text-slate-700">Deskripsi
                                Meta</label>
                ...
                <button type="submit" id="btn-save"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                        ...
                        stroke-linejoin="round" class="mr-3 group-hover:rotate-12 transition-transform">
...
@push('scripts')
    ...
    <script>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            submitBtn.innerHTML = '<span class="animate-spin mr-3">◌</span> Menyimpan...';
            ...
            try {
                const res = await axios.post(`/cyber-security-services/${serviceId}`, fd);
                showToast('success', 'Pembaruan Berhasil', 'Detail layanan telah diperbarui dan sinkron.');
                setTimeout(() => window.location.href = '/cyber-security-services', 1500);
            } catch (err) {
                console.error(err);
                const errors = err.response?.data?.errors;
                let msg = 'Gagal memperbarui detail layanan.';
                if (errors) msg = Object.values(errors).flat().join('<br>');
                showToast('error', 'Pembaruan Gagal', msg);
            } finally {
